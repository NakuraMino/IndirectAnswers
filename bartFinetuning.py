...
from transformers import AutoTokenizer, AutoModel, AdamW, BartTokenizer, BartForSequenceClassification, BartConfig
...
def main():
    ...
    if args.prev_labels != 0:
        config = BartConfig.from_pretrained(args.model_type, num_labels=args.prev_labels) 
    else:
        config = BartConfig.from_pretrained(args.model_type, num_labels=args.num_labels)
    

    tokenizer = BartTokenizer.from_pretrained(args.model_type)        
    
    model = BartForSequenceClassification.from_pretrained(args.model_type, config=config)
