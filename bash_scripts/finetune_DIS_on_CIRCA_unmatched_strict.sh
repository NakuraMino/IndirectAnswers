...
## File for finetuning the DIS model on the unmatched, strict version of the CIRCA dataset
...
## For label of size 6
echo "Finetuning DIS CIRCA with unmatched, strict labels..."
...
for j in $ScenarioNum; do # 10 iterations, leaving a different scenario out each time
    ...
    python finetuning.py \
        ...
        --model_name "DIS_CIRCA_BERT_unmatched_strict_s${j}_e3_lr${lr}_b32" \
        --dataset_type "CIRCA" \
        --transfer_on \
        --batch_size "32" \
        --epochs "3" \
...
echo "Finished finetuning DIS CIRCA with unmatched, strict labels..."
