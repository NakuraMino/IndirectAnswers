...
## File for finetuning the SNLI model on the unmatched, relaxed version of the CIRCA dataset
...
## For label of size 6
echo "Finetuning SNLI CIRCA with unmatched, relaxed labels..."
...
for j in $ScenarioNum; do # 10 iterations, leaving a different scenario out each time
    ...
    python finetuning.py \
        ...
        --model_name "SNLI_CIRCA_BERT_unmatched_relaxed_s${j}_e3_lr${lr}_b32" \
        --dataset_type "CIRCA" \
        --transfer_on \
        --batch_size "32" \
        --epochs "3" \
...
echo "Finished finetuning SNLI CIRCA with unmatched, relaxed labels..."
