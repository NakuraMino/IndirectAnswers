...
## File for testing on the unmatched, strict dataset
...
echo "Testing on SNLI (pretrained) CIRCA with unmatched, strict labels..."
...
for j in $ScenarioNum; do # 10 iterations, leaving a different scenario out each time
    ...
    --model_type "$BASE_DIR/models/SNLI_CIRCA_BERT_unmatched_strict_s${j}_e3_lr${lr}_b32" \
    --dataset_type "CIRCA" \
    --batch_size "32" \
    --num_labels "9" \
...
echo "Finished testing SNLI (pretrained) CIRCA with unmatched, strict labels..."
